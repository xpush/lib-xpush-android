    ...
    private void onPacket(Packet packet) {
        if (this.readyState == ReadyState.OPENING || this.readyState == ReadyState.OPEN) {
            ...
            if (Packet.OPEN.equals(packet.type)) {
                try {
                    logger.info(String.format("handshake : '%s'", packet.data));
                ...
                logger.info(String.format("PONG : '%s'", packet.data));
                this.setPing();
        ...
        this.pingIntervalTimer = this.getHeartbeatScheduler().schedule(new Runnable() {
            ...
            public void run() {
                EventThread.exec(new Runnable() {
                    @Override
                    public void run() {
                        logger.info(String.format("writing ping packet - expecting pong within %sms", self.pingTimeout) + " : " + String.valueOf( self.pingInterval ) );
                        self.ping();
